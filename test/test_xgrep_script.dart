...
runScriptWithArgs(List<String> args) async =>
  await Process.run(xgrep, args, environment:env)
  .then((ProcessResult result) => result.stdout);

runScriptWithArgsInteractive(List<String> args, [interaction(Process)]) async =>
  await Process.start(xgrep, args, environment:env)
  .then((Process process) {
    ...
    return process
      .exitCode
      .then((int exitCode) => expect(exitCode, 0));
  });

addTest(testName, scriptArgs, required) =>
  test(testName, () async =>
    await runScriptWithArgs(scriptArgs)
      .then((String output) {
        if(true) {
          print('--------COMPLETED $testName------');
          print(output);
        }
        expect(required.every((s) => output.contains(s)), true);
      }));
...
// custom <main>
  ...
  group('test_xgrep_script', () {
    ...
    test('remove_all', () async {
      await runScriptWithArgsInteractive(['-R'],
          (process) => process.stdin.writeln('Y'))
      .then((var _) async {
    ...
    addTest('-i with -p creates index',
        ['-i', 'test_index', '-p', '$testFolder'],
        [ 'Creating/updating test_index' ]);

    addTest('without args displays index',
        [],
        ['--------- test_index']);

    addTest('without -h does help',
        ['-h'],
        ['-l, --[no-]list']);
    ...
    addTest('creation honors prune',
        ['-i', 'test_index', '-p', '$rootFolder:prune_test'],
        [ 'Creating/updating test_index',
          new RegExp(r'"names":\s*\[\s*"prune_test"', multiLine:true) ]);

    addTest('grep works',
        ['-i', 'test_index', 'funky_monkey', 'spinal_tap'],
        [
    ...
    addTest('create a second index',
        ['-i', 'test_index2', '-p', binFolder],
        ['Creating/updating test_index2']);

    addTest('grep looks in multiple indices',
        ['-i', 'test_index', '-i', 'test_index2' 'funky_monkey', 'Are you sure'],
        [
          // From test_index
          new RegExp(r'test_xgrep_script.dart:\d+.*funky_monkey'),
          // From test_index2
